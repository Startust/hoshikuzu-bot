...
import { diffByUsername } from '../services/flyffRanking/diff';
import { fetchAllPlayers } from '../services/flyffRanking/scrape';
import { listAllConfigs, // 新增：返回所有有配置的 guild（哪怕没 enabled）
listWatchedGuilds, loadSnapshot, saveSnapshot, } from '../services/flyffRanking/store';
const running = new Set();
const nextRunAt = new Map();
export function startFlyffPoller(client) {
    setInterval(async () => {
        ...
        for (const cfg of configs) {
            ...
            try {
                console.log(`[flyff poller] running for guild ${key} at ${new Date().toISOString()}`);
                ...
                await saveSnapshot(cfg.discordGuildId, cfg.flyffServerId, latest.map((p) => ({
                    username: p.username,
                    rank: p.rank,
                    level: p.level,
                    job: p.job,
                ...
                running.delete(key);
                console.log(`[flyff poller] next run for guild ${key} at ${new Date().toISOString()}`);
            }
        }
    }, 30_000);
}
//# sourceMappingURL=flyff-poller.js.map