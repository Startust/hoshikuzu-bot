...
function buildUrl(serverId, page) {
...
function parsePlayersFromHtml(html) {
    ...
    rows.each((_, tr) => {
        ...
        if (rankText) {
            ...
            if (medalAlt.includes('first'))
                rank = 1;
            else if (medalAlt.includes('second'))
                rank = 2;
            else if (medalAlt.includes('third'))
                rank = 3;
        }
...
export async function fetchAllPlayers(serverId) {
